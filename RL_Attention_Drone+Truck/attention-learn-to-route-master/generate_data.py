...
def generate_vrp_data(dataset_size, vrp_size):
    ...
    if vrp_size==10:
        uselist=np.random.uniform(1,6,size=(10000,2)).tolist()
        data_vrp_10=[]
        cov=[[0.2,0],[0,0.1]]
        for i in range(10000):
            data_vrp_10.append(np.random.multivariate_normal(uselist[i], cov, size=10).tolist())
        cvrp10_data = list(zip(
            uselist,  # Depot location
            data_vrp_10,  # Node locations
            np.random.randint(1, 10, size=(10000, 10)).tolist(),  # Demand, uniform integer 1 ... 9
            np.full(10000, 20).tolist()  # Capacity, same for whole dataset
        ))
